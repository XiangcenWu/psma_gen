...
def save_mi_ncc_txt(filename: str, mi_list, ncc_list, fmt: str = "{:.6f}"):
    mi_line = " ".join(fmt.format(x) for x in mi_list)
    ncc_line = " ".join(fmt.format(x) for x in ncc_list)
...
def main(args):
    ...
    if os.path.isdir(args.weights_path):
        ...
        for weights_path in weights_paths_list:
            ...
            print(f'inference model name: {path}')

            filename = os.path.join('/share/home/xcwu/pet_reg_results_whole_body', path)


            model.load_state_dict(torch.load(weights_path, map_location=device))
            mi_list, ncc_list = inference_batch_whole_body(
                model,
                test_loader,
                identity_grid,
                device=args.device
            ...
            save_mi_ncc_txt(filename, mi_list, ncc_list)
